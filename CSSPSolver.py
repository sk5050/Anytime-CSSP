...
class CSSPSolver(object):
    def __init__(
        self, model, VI_epsilon=1e-50, VI_max_iter=100000, convergence_epsilon=1e-50, resolve_epsilon=1e-5, bounds=[]
    ):
        self.model = model
        self.bounds = bounds
    ...
    def find_dual(self, initial_alpha_set):
        ...
        value_1, value_2, value_3, value_4, value_5, value_6, value_7, value_8, value_9 = self.algo.get_values(
            self.algo.graph.root
        )
        weighted_value = self.algo.compute_weighted_value(
    ...
    def find_dual_multiple_bounds_generalized(self, initial_alpha_set, M=6):
        ...
        L_new = (
            f_plus
            + self.algo.alpha[0] * g_plus[0]
            + self.algo.alpha[1] * g_plus[1]
            + self.algo.alpha[2] * g_plus[2]
            + self.algo.alpha[3] * g_plus[3]
            + self.algo.alpha[4] * g_plus[4]
            + self.algo.alpha[5] * g_plus[5]
        )
        while True:
            L_prev = L_new
            ...
            for bound_idx in range(len(initial_alpha_set)):  # looping over each bound (coorindate)
                ...
                while True:
                    ...
                    if g_minus[bound_idx] == g_plus[bound_idx]:
                        ...
                        L_u = (
                            f
                            + self.algo.alpha[0] * g[0]
                            + self.algo.alpha[1] * g[1]
                            + self.algo.alpha[2] * g[2]
                            + self.algo.alpha[3] * g[3]
                            + self.algo.alpha[4] * g[4]
                            + self.algo.alpha[5] * g[5]
                        )
            ...
            for state, head, blocked_action_set in candidate_generating_states:
                ...
                if new_candidate:
                    if self.candidate_exists(new_candidate):
                        self.return_to_best_graph(
                            self.algo.graph, current_best_graph
                        )  ## returning to the previous graph.
                        continue
                    else:
    ...
    def find_next_best(self):
        ...
        self.solution_history.append((current_best_weighted_value, current_best_values, current_best_policy))
    ...
    def prune_candidates(self, current_best_policy, given_epsilon):
        if self.candidate_pruning == False:
            ...
            initial_pruned_states = [
                (state, state_idx_dict[state])
                for state in sorted_states
                if N_vector[state_idx_dict[state]] < initial_epsilon
            ]
